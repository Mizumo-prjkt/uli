...
#include "contents/translations/lang_export.hpp" // Langexpert is based on the translaions
...
public:
  ...
  static std::string format_menu_item(const std::string &label,
  ...
  // Renders the overarching config menu modeled after archinstall (now
  // interactive)
  static std::string render_main_menu(const MenuState &state,
                                      int &last_selected,
                                      const std::string &os_distro) {
    std::vector<std::string> options;
    std::vector<std::string> actions;

    auto add_opt = [&](const std::string &a, const std::string &l,
                       const std::string &v) {
    ...
    std::string mirror_display =
        state.active_mirrors.empty()
            ? "None"
            : (state.active_mirrors.size() == 1
                   ? state.active_mirrors[0]
                   : std::to_string(state.active_mirrors.size()) + " selected");
    add_opt("MIRROR", "Mirror region", mirror_display);

    add_opt("REPOS", "Optional repositories",
            state.get_optional_repos_str(os_distro));
    ...
    std::string root_pwd_display =
        (state.root_password == "None" || state.root_password.empty()) ? "None"
                                                                       : "Set";
    ...
    add_opt("NTP", "Automatic time sync (NTP)",
            (state.ntp_sync ? "True" : "False"));

    options.push_back("");
    actions.push_back("SPACER");

    options.push_back(std::string(DesignUI::DIM) + "> " +
                      _tr("Save configuration") + DesignUI::RESET);
    actions.push_back("SAVE");

    if (state.drive.empty()) {
      options.push_back(std::string(DesignUI::DIM) + "> " +
                        _tr("Install (1 config(s) missing)") + DesignUI::RESET);
    } else {
      options.push_back(std::string(DesignUI::GREEN) + "> " + _tr("Install") +
                        DesignUI::RESET);
    }
    actions.push_back("INSTALL");

    options.push_back(std::string(DesignUI::RED) + "> " + _tr("Abort") +
                      DesignUI::RESET);
    actions.push_back("ABORT");

    int sel = UIMenuDesign::show_interactive_list(
        "Universal Linux Installer Engine", options, last_selected);
    if (sel != -1) {
      last_selected = sel;
      return actions[sel];
    }
