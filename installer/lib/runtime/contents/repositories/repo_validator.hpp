...
class RepoValidator {
public:
    // Validates optional_repos entries loaded from YAML against the current distro context.
    // Called after loading a YAML config, before the interactive menu loop.
    static void validate_repos(MenuState& state, const std::string& os_distro) {
        if (state.optional_repos.empty()) return;

        // ──────────────────────────────────────────────────
        // 1. Non-Debian/Ubuntu distro with Debian-style repos → ignore + alert
        // ──────────────────────────────────────────────────
        if (os_distro != "Debian" && os_distro != "Ubuntu") {
            ...
            if (has_apt_repos) {
                DialogBox::show_alert(
                    ...
                    "but you are installing " + os_distro + ".\n\n"
                    "These repository entries will be IGNORED.\n"
                    "Only native package sources for " + os_distro + " are supported."
                );
            ...
            if (os_distro != "Alpine Linux") {
                ...
                if (has_alpine_repos) {
                    DialogBox::show_alert(
                        "\033[1;33;41m ⚠ INCOMPATIBLE REPOSITORIES ⚠ \033[0m",
                        "Your YAML configuration contains Alpine apk-style repository entries,\n"
                        "but you are installing " + os_distro + ".\n\n"
                        "These repository entries will be IGNORED.\n"
                        "Alpine packages (.apk) are NOT compatible with other distributions."
                    );
                    std::vector<std::string> filtered;
                    for (const auto& repo : state.optional_repos) {
                        if (repo.find("APK|") != 0 &&
        ...
        if (ver >= 13) {
            ...
            for (const auto& repo : state.optional_repos) {
                if (repo.find("CLASSIC|") == 0) { has_classic = true; break; }
            }
            if (has_classic) {
                DialogBox::show_alert(
                    "\033[1;33;43m ⚠ DEPRECATION WARNING ⚠ \033[0m",
                    "Your YAML config contains classic sources.list entries,\n"
                    "but Debian " + std::to_string(ver) + " uses the deb822 format.\n\n"
            ...
            for (const auto& repo : state.optional_repos) {
                if (repo.find("DEB822|") == 0) { has_deb822 = true; break; }
            }
            if (has_deb822) {
                DialogBox::show_alert(
                    "\033[1;31;47m ✖ INCOMPATIBLE FORMAT ✖ \033[0m",
                    "Your YAML config contains deb822 format repository entries,\n"
                    "but you are targeting Debian " + std::to_string(ver) + " which does NOT\n"
                ...
                std::vector<std::string> filtered;
                for (const auto& repo : state.optional_repos) {
                    if (repo.find("DEB822|") != 0) filtered.push_back(repo);
                }
                state.optional_repos = filtered;
            }
        }
        ...
        // ──────────────────────────────────────────────────
        // 3. Cross-distro URL detection (Ubuntu ↔ Debian ↔ Alpine ↔ Arch)
        // ──────────────────────────────────────────────────
        for (const auto& repo : state.optional_repos) {
            bool is_debian_url = (repo.find("deb.debian.org") != std::string::npos || 
                                  repo.find("security.debian.org") != std::string::npos);
            bool is_ubuntu_url = (repo.find("archive.ubuntu.com") != std::string::npos ||
                                  repo.find("security.ubuntu.com") != std::string::npos);
            bool is_alpine_url = (repo.find("dl-cdn.alpinelinux.org") != std::string::npos ||
                                  repo.find("alpine/") != std::string::npos);
            bool is_arch_url   = (repo.find("archlinux.org") != std::string::npos ||
                                  repo.find("mirror.rackspace.com/archlinux") != std::string::npos);

            std::string foreign_source;
            if (os_distro == "Ubuntu" && is_debian_url) foreign_source = "deb.debian.org (Debian)";
            else if (os_distro == "Ubuntu" && is_alpine_url) foreign_source = "Alpine Linux";
            else if (os_distro == "Ubuntu" && is_arch_url) foreign_source = "Arch Linux";
            else if (os_distro == "Debian" && is_ubuntu_url) foreign_source = "archive.ubuntu.com (Ubuntu)";
            else if (os_distro == "Debian" && is_alpine_url) foreign_source = "Alpine Linux";
            else if (os_distro == "Debian" && is_arch_url) foreign_source = "Arch Linux";
            else if (os_distro == "Alpine Linux" && is_debian_url) foreign_source = "Debian";
            else if (os_distro == "Alpine Linux" && is_ubuntu_url) foreign_source = "Ubuntu";
            else if (os_distro == "Alpine Linux" && is_arch_url) foreign_source = "Arch Linux";
            else if (os_distro == "Arch Linux" && is_debian_url) foreign_source = "Debian";
            else if (os_distro == "Arch Linux" && is_ubuntu_url) foreign_source = "Ubuntu";
            else if (os_distro == "Arch Linux" && is_alpine_url) foreign_source = "Alpine Linux";

            if (!foreign_source.empty()) {
                DialogBox::show_alert(
                    "\033[1;37;41m ⚠⚠⚠ CROSS-DISTRO REPOSITORY WARNING ⚠⚠⚠ \033[0m",
                    "A repository URL points to " + foreign_source + " but you are\n"
                    "installing " + os_distro + ".\n\n"
                    ...
                    "native to " + os_distro + " will be available.\n"
                    "You can still add packages manually via 'Additional packages'.\n\n"
    ...
    // ──────────────────────────────────────────────────
    static std::vector<std::pair<std::string, std::string>> auto_guess_packages(
        ...
        struct Mapping { std::string from; std::string to; std::string src; std::string tgt; };
    ...
    static void show_auto_guess_dialog(
        const std::vector<std::string>& packages,
        const std::string& source_distro,
        const std::string& target_distro
    ) {
        ...
        DialogBox::show_alert(
            "\033[1;36m Package Translation Suggestions \033[0m",
            body
        );
    }
};
