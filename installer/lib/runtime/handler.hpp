...
public:
  ...
  static std::string handle_disk_configuration(MenuState &state) {
    while (true) {
      ...
      int sel = DialogBox::ask_selection(_tr("Storage Configuration"),
                                         _tr("Select an installation target:"),
                                         disk_choices);
      if (sel == -1 || sel == (int)disk_choices.size() - 1)
        return "";

      if (disks[sel].size == "0B" || disks[sel].size == "0") {
        DialogBox::show_alert(_tr("Error"),
                              _tr("This is a problematic block partition (0B) "
      ...
      while (!back_to_disks) {
        if (state.drive == selected_disk && !state.partitions.empty()) {
          auto ex_banner_func = [&state, selected_disk]() {
            return ManualPartitionWizard::get_dynamic_layout_banner(
                selected_disk, state);
          };
          std::vector<std::string> ex_choices = {
              _tr("Keep current layout and return to menu"),
              _tr("Modify layout manually"), _tr("Clear layout and start over"),
              _tr("Cancel / Back to Disk Selection")};
          int ex_sel = DialogBox::ask_selection(
              _tr("Existing Layout Found"),
              _tr("You have already configured partitions for this drive."),
              ex_choices, ex_banner_func);
        ...
        auto banner_func = [selected_disk]() {
          return uli::runtime::contents::drive::DiskGraph::
              get_disk_layout_string(selected_disk);
        };
        ...
        int part_sel = DialogBox::ask_selection(
            _tr("Partitioning"),
            _tr("Select partitioning method for ") + selected_disk,
            part_choices, banner_func);
        ...
        if (part_sel == 0 || part_sel == 1) {
          ...
          std::vector<std::string> strategy_choices = {
              ...
              _tr("Cancel / Back")};
          int strat_sel = DialogBox::ask_selection(
              _tr("Guided Strategy"), _tr("Select division strategy:"),
              strategy_choices, banner_func);
          ...
          long long total_disk =
              uli::partitioner::DiskCheck::get_disk_size_bytes(selected_disk);
          long long used_bytes = 0;

          if (!temp_state.purge_disk_intent) {
            auto existing_parts =
                uli::partitioner::DiskCheck::get_disk_partitions(selected_disk);
            int pnum = 1;
          ...
          long long available_bytes =
              total_disk - used_bytes - (2 * 1024 * 1024);    // 2MB padding
          long long absolute_min = 24LL * 1024 * 1024 * 1024; // 24GB

          if (available_bytes < absolute_min) {
            DialogBox::show_alert(_tr("Error"),
                                  _tr("Intelligent Layout Failed: Insufficient "
                                      "Free Space (Requires 24GB Minimum)."),
                                  banner_func);
            continue;
          }
          ...
          int logic_path =
              0; // 0=Unified, 1=Root+Home, 2=Root+Var+Home, 3=Root+Opt+Var+Home
          if (strat_sel == 0) { // Auto-compute
            if (available_bytes < gb14)
              logic_path = 0;
            else if (available_bytes < gb150)
              logic_path = 1;
            else
              logic_path = 2;
          } else if (strat_sel == 1)
          ...
          if (available_bytes >
              10LL * 1024 * 1024 * 1024) { // Only suggest if > 10GB
            std::vector<std::string> swap_choices = {
                _tr("Include 4GiB Swap Partition"),
                _tr("No Swap Partition (Acknowledge)")};
            int swap_sel = DialogBox::ask_selection(
                _tr("Swap Configuration"),
                _tr("Do you want to include a swap partition?"), swap_choices,
                banner_func);
            if (swap_sel == 0)
              include_swap = true;
            else if (swap_sel == -1)
          ...
          auto preview_banner = [&temp_state, selected_disk]() {
            std::string banner =
                ManualPartitionWizard::get_dynamic_layout_banner(selected_disk,
                                                                 temp_state);
            bool has_swap = false;
            for (const auto &p : temp_state.partitions)
              if (p.fs_type == "swap")
                has_swap = true;
            if (!has_swap) {
              banner += "\n\n" + std::string(DesignUI::YELLOW) +
                        _tr("!! WARNING: No Swap partition configured. !!") +
                        DesignUI::RESET;
            }
            return banner;
          };

          std::vector<std::string> confirm_choices = {_tr("Yes, Apply Layout"),
                                                      _tr("No, Cancel")};
          int confirm_sel = DialogBox::ask_selection(
              _tr("Layout Preview"), _tr("Apply this layout to the disk?"),
  ...
  // Process all pending deferred partition configurations globally before
  // install unpacks
  static bool execute_deferred_partitions(MenuState &state) {
    BlackBox::log("ENTER execute_deferred_partitions");
    if (state.partitions.empty()) {
      BlackBox::log("No partitions to execute");
      return true;
    }

    if (state.drive.empty() || state.drive == "None") {
      Warn::print_error("Cannot execute partitions: No target drive selected!");
      BlackBox::log("ERROR: No target drive selected");
      return false;
    }
    ...
    for (const auto &p : state.partitions)
      if (p.is_deferred)
        has_deferred = true;
    if (!has_deferred) {
      BlackBox::log("No deferred partitions found");
      return true;
    }

    Warn::print_info("Executing deferred partition layouts onto \"" +
                     state.drive + "\"...");
    BlackBox::log("Starting GPT initialization on " + state.drive);
    std::cout
        << "[partitioner] Initializing new GUID Partition Table (GPT)...\n";
    if (!uli::partitioner::sgdisk::SgdiskWrapper::create_gpt_table(
            state.drive)) {
      Warn::print_error("Failed to initialize GPT on " + state.drive);
      BlackBox::log("FATAL: GPT initialization failed");
      return false;
    }
    ...
    for (auto &p : state.partitions) {
      ...
      BlackBox::log("Creating partition " + std::to_string(p.part_num) +
                    " size " + p.size_cmd);
      std::cout << "[partitioner] Slicing partition " << p.part_num << " ("
                << p.size_cmd << ")...\n";
      bool success = uli::partitioner::sgdisk::SgdiskWrapper::create_partition(
          state.drive, p.part_num, "0", p.size_cmd, p.type_code);
      if (!success) {
        Warn::print_error("Failed to construct partition " +
                          std::to_string(p.part_num));
        BlackBox::log("ERROR: Partition creation failed for index " +
                      std::to_string(p.part_num));
        return false;
      }
      ...
      p.partuuid = uli::partitioner::identifiers::BlkidWrapper::get_partuuid(
          dev_part_path);
      p.is_deferred = false;
      std::cout << "[partitioner] SUCCESS: " << dev_part_path
                << " ready. PARTUUID=" << p.partuuid << "\n";
      BlackBox::log("Partition " + dev_part_path +
                    " success. PARTUUID=" + p.partuuid);
    }
    BlackBox::log("EXIT execute_deferred_partitions success");
    return true;
  }
  ...
  static bool mount_all_partitions(MenuState &state,
    ...
    std::sort(sorted_parts.begin(), sorted_parts.end(),
              [](const PartitionConfig &a, const PartitionConfig &b) {
                if (a.fs_type == "swap" && b.fs_type != "swap")
                  return true;
                if (a.fs_type != "swap" && b.fs_type == "swap")
                  return false;
  ...
  static bool generate_fstab(const std::string &mount_point) {
    std::cout << "[fstab] Generating /etc/fstab for target " << mount_point
              << "..." << std::endl;
    std::string cmd = "genfstab -U " + mount_point + " >> " + mount_point +
  ...
  handle_package_search(const std::string &active_distro,
    ...
    if (active_distro == "Debian") {
      ...
      DialogBox::show_alert(
          "Index Error", "Unsupported package manager for this distribution.");
      return current_pkgs;
    }

    // Failsafe Ping
    std::vector<std::string> test_ping = pm->search_packages(
        ""); // empty query returns empty gracefully, let's explicitly ping
             // 'nano' to test binary presence
    test_ping = pm->search_packages("nano");

    if (!test_ping.empty() && test_ping[0] == "_ULI_PM_ERROR_") {
      DialogBox::show_alert(
          "Index Error",
          "The underlying package manager failed to communicate. Indexing "
          "packages will not work. Expect issues on the install run.");
      std::string man_input = DialogBox::ask_input(
          "Manual Package Entry",
          "Search is offline. Type explicit packages separated by "
          "spaces.\n\nWARNING: Ensure strict spelling or else the install may "
          "fail at runtime! You are on your own.");
      if (!man_input.empty()) {
        std::istringstream iss(man_input);
        std::string pkg;
        while (iss >> pkg) {
          if (std::find(current_pkgs.begin(), current_pkgs.end(), pkg) ==
              current_pkgs.end()) {
