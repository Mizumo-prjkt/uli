#include "debug_repo_fetcher.hpp"
#include <map>
...
bool ArchDebugFetcher::fetch_and_parse(const std::string &url) {
  ...
  while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
    ...
    if (entryPath.size() > 5 &&
        entryPath.substr(entryPath.size() - 5) == "/desc") {
...
#else
  std::cerr
      << "[ERROR] libarchive support NOT compiled in. Arch fetching disabled."
      << std::endl;
  return false;
#endif
}
...
bool DebianDebugFetcher::fetch_and_parse(const std::string &url) {
  ...
  while (std::getline(full_stream, line)) {
    ...
    if (line.find("Package: ") == 0)
      pkg.name = line.substr(9);
    else if (line.find("Version: ") == 0)
      pkg.version = line.substr(9);
    else if (line.find("Description: ") == 0)
      pkg.description = line.substr(13);
...
bool AlpineDebugFetcher::fetch_and_parse(const std::string &url) {
  ...
  while (std::getline(full_stream, line)) {
    ...
    if (line.find("P:") == 0)
      pkg.name = line.substr(2);
    else if (line.find("V:") == 0)
      pkg.version = line.substr(2);
    else if (line.find("T:") == 0)
      pkg.description = line.substr(2);
