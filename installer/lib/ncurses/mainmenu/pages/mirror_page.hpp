...
public:
  ...
  std::string title() const override {
    switch (section_) {
    case MirrorSection::Reflector:
      return "Mirrors > Auto Fetch";
  ...
  void render(WINDOW *win) override {
    switch (section_) {
    case MirrorSection::Main:
      render_main(win);
      break;
    case MirrorSection::Reflector:
      render_reflector(win);
      break;
    case MirrorSection::Manual:
      render_manual(win);
      break;
  ...
  bool handle_input(WINDOW *win, int ch) override {
    ...
    if (NcursesLib::is_back_key(ch)) {
      // Intercept to only unfocus the list if we're in the Manual section list
      // mode
    ...
    switch (section_) {
    case MirrorSection::Main:
      return handle_main(ch);
    case MirrorSection::Reflector:
      return handle_reflector(ch);
    case MirrorSection::Manual:
      return handle_manual(win, ch);
    case MirrorSection::Settings:
      return handle_settings(ch);
    }
    return false;
  }
...
private:
  ...
  void render_reflector(WINDOW *win) {
    ...
    mvwprintw(win, 3, 4, "Countries:      %s",
              DataStore::instance().reflector_cfg.countries.c_str());
    mvwprintw(win, 5, 4, "Number (Last):  %d",
              DataStore::instance().reflector_cfg.last_n);
    mvwprintw(win, 7, 4, "Sort Method:    %s",
  ...
  void render_manual(WINDOW *win) {
    ...
    // Action Options
    const char *actions[] = {
        "Input URL", "Add New Repository", "Edit Added Repositories",
        "Clear All", "[ SAVE & BACK ]",    "[ DO NOT SAVE & BACK ]"};
    ...
    if (manual_list_.is_focused) {
      ...
      mvwprintw(
          win, h - 2, 2,
          "[UP/DOWN: Scroll] [E: Edit] [D: Delete] [BACKSPACE: Exit List]");
  ...
  bool handle_main(int ch) {
    ...
    if (ch == KEY_DOWN && selected_ < 2) {
      selected_++;
      return true;
    }
    if (ch == '\n' || ch == KEY_ENTER) {
      if (selected_ == 0)
        section_ = MirrorSection::Reflector;
      else if (selected_ == 1)
        section_ = MirrorSection::Manual;
      else if (selected_ == 2)
        section_ = MirrorSection::Settings;
      selected_ = 0;
      return true;
    }
    return false;
  ...
  bool handle_manual(WINDOW *win, int ch) {
    if (manual_list_.is_focused) {
      if (ch == 'e' || ch == 'E' || ch == '\n' || ch == KEY_ENTER) {
        if (!manual_list_.items.empty()) {
          std::string current_val =
              manual_list_.items[manual_list_.selected_idx];
          std::string edited =
              InputPopup::show("Edit Mirror", "Repository URL:", current_val);
          if (!edited.empty()) {
            if (is_valid_protocol(edited))
              manual_list_.items[manual_list_.selected_idx] =
                  normalize_url(edited);
            else
              ErrorPopup::show("Validation Error",
                               "Invalid protocol (https/ftp/rsync required)");
          }
        }
        return true;
      }
      if (ch == 'd' || ch == 'D' || ch == KEY_DC) {
        if (!manual_list_.items.empty()) {
          if (YesNoPopup::show(
                  "Confirm Delete",
                  "Are you sure you want to remove this mirror?")) {
            manual_list_.items.erase(manual_list_.items.begin() +
                                     manual_list_.selected_idx);
            if (manual_list_.selected_idx > 0 &&
                manual_list_.selected_idx >= (int)manual_list_.items.size()) {
              manual_list_.selected_idx--;
            }
          }
        }
    ...
    if (ch == '\n' || ch == KEY_ENTER) {
      if (selected_ == 0) {
        manual_url_ = InputPopup::show("Manual Input",
                                       "Enter Repository URL:", manual_url_);
        ...
        if (YesNoPopup::show(
                "Confirm Clear",
                "Are you sure you want to clear all added mirrors?"))
          manual_list_.items.clear();
      } else if (selected_ == 4) {
  ...
  bool handle_settings(int ch) {
    ...
    if (ch == '\n' || ch == KEY_ENTER || ch == ' ') {
      if (selected_ < 4) {
        ...
        // but since the user approved FormPopup, let's use it for a "Configure
