...
void handle_terminal_resize(int) {
  // Dummy handler emitting EINTR to interrupt read() causing UI logic to
  // re-render
}
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
      ...
      std::cout << "ULI Installer version " << uli::PROJECT_VERSION << " ("
                << uli::BUILD_TYPE << ")" << std::endl;
      return 0;
#ifdef ULI_DEBUG_MODE
    } else if (arg == "--test-memleak") {
      std::cout << "[MEMLEAK] Starting memory leak diagnostic loop. PID: "
                << getpid() << std::endl;
      ...
      if (current_distro == uli::checks::DistroType::DEBIAN) {
        ...
        std::cerr << "[MEMLEAK] Cannot test on unknown distribution."
                  << std::endl;
        return 1;
      }

      // Adjust path depending on where it's run
      std::string trans_yaml_path =
      ...
      if (!pm->load_translation(trans_yaml_path)) {
        if (bootargs.translation_yaml.empty()) {
          ...
          std::cerr << "[MEMLEAK] Failed to load custom trans.yaml: "
                    << trans_yaml_path << std::endl;
          return 1;
        }
      }

      std::vector<std::string> abstract_pkgs = {
          "nginx", "mysql", "base_devel", "python", "custom_pkg", "aur-yay"};
      ...
      while (true) {
        ...
        iter++;
        if (iter % 1000 == 0) {
          std::cout << "[MEMLEAK] Completed " << iter
                    << " translation operations." << std::endl;
        }
  ...
  if (run_lint) {
    ...
    if (final_profile.empty())
      final_profile = bootargs.config_file;

    if (final_profile.empty()) {
      std::cerr << "[ERROR] --lintcheck requires a profile. Use --profile "
                   "<path> or uli.yaml"
                << std::endl;
      return 1;
    }
    ...
    if (final_dict.empty()) {
      if (std::filesystem::exists(
        ...
        using_builtin = true;
        if (current_distro == uli::checks::DistroType::ARCH)
          final_dict = "BUILTIN_TRANS_ARCH";
        else if (current_distro == uli::checks::DistroType::ALPINE)
          final_dict = "BUILTIN_TRANS_ALPINE";
        else
          final_dict = "BUILTIN_TRANS_DEBIAN";
      }
    }

    auto issues = uli::runtime::config::YamlLinter::lint(
        final_profile, final_dict, dict_provided_explicitly, using_builtin);
    uli::runtime::config::YamlLinter::print_report(issues);

    if (uli::runtime::config::YamlLinter::has_fatal(issues))
      return 1;
    return 0;
  }
  ...
  if (uli::runtime::TestSimulation::get_config().fetch_repos) {
    std::string distro =
        uli::runtime::TestSimulation::get_config().distribution;
    if (distro.empty()) {
      if (current_distro == uli::checks::DistroType::ARCH)
        distro = "arch";
      else if (current_distro == uli::checks::DistroType::ALPINE)
    ...
    std::cout << "[DEBUG] Starting repo fetch for distribution: " << distro
              << std::endl;
    auto fetcher = uli::debug::FetcherFactory::create(distro);
    if (fetcher) {
      std::vector<std::string> urls;
      if (distro == "arch") {
        urls = {"https://mirror.osbeck.com/archlinux/core/os/x86_64/core.db",
                "https://mirror.osbeck.com/archlinux/extra/os/x86_64/extra.db"};
      } else if (distro == "alpine") {
        urls = {"https://dl-cdn.alpinelinux.org/alpine/v3.19/main/x86_64/"
                "APKINDEX.tar.gz",
                "https://dl-cdn.alpinelinux.org/alpine/v3.19/community/x86_64/"
                "APKINDEX.tar.gz"};
      } else {
        urls = {"http://ftp.debian.org/debian/dists/trixie/main/binary-amd64/"
                "Packages.xz",
                "http://security.debian.org/debian-security/dists/"
                "trixie-security/main/binary-amd64/Packages.xz",
                "http://ftp.debian.org/debian/dists/trixie-updates/main/"
      ...
      std::cout << "[DEBUG] Total packages found across all repos: "
                << pkgs.size() << std::endl;
      if (!pkgs.empty()) {
        std::cout << "[DEBUG] Sample package: " << pkgs[0].name << " ("
                  << pkgs[0].version << ")" << std::endl;
      }
    } else {
      std::cerr << "[ERROR] No fetcher available for distro: " << distro
                << std::endl;
    }
  ...
  if (detected_debian_version == 0 &&
      current_distro == uli::checks::DistroType::DEBIAN) {
    detected_debian_version =
        uli::checks::DistroVersion::detect_major_version();
  }
  ...
  if (current_distro == uli::checks::DistroType::DEBIAN) {
    ...
    std::cerr << "Error: Unsupported or unknown Linux distribution."
              << std::endl;
    return 1;
  }
  ...
  if (uli::env::RuntimeEnv::is_live_environment()) {
    std::cout << "[INFO] Detected Execution in Live CD/USB Environment."
              << std::endl;
  } else {
  ...
  if (!final_dict_path.empty()) {
    if (pm->load_translation(final_dict_path)) {
      ...
      std::cerr << "[ERROR] Could not load specified dictionary: "
                << final_dict_path << std::endl;
      return 1;
    }
  }
    ...
    for (const auto &path : search_paths) {
      if (std::filesystem::exists(path)) {
        ...
        try {
          ...
          if (node["distro_id"])
            yaml_distro = node["distro_id"].as<std::string>();

          bool match = false;
          if (current_distro == uli::checks::DistroType::ARCH &&
              yaml_distro == "arch")
            match = true;
          else if (current_distro == uli::checks::DistroType::DEBIAN &&
                   yaml_distro == "debian")
            match = true;
          else if (current_distro == uli::checks::DistroType::ALPINE &&
                   yaml_distro == "alpine")
          ...
          if (match && pm->load_translation(path)) {
            std::cout << "[INFO] Found matching external dictionary at: "
                      << path << std::endl;
            loaded = true;
            break;
          }
  ...
  if (!loaded && !disable_builtin_dict) {
    std::cout << "[INFO] Using embedded multi-distro fallback for "
              << distro_name << std::endl;
    std::string builtin_str;
    if (current_distro == uli::checks::DistroType::ARCH)
      builtin_str = uli::package_mgr::BUILTIN_TRANS_ARCH;
    else if (current_distro == uli::checks::DistroType::ALPINE)
      builtin_str = uli::package_mgr::BUILTIN_TRANS_ALPINE;
    else
      builtin_str = uli::package_mgr::BUILTIN_TRANS_DEBIAN;
  ...
  if (!loaded) {
    std::cerr << "[FATAL] No translation dictionary loaded. Installation "
                 "cannot proceed."
              << std::endl;
    return 1;
  }
  ...
  if (!bootargs.unattended) {
    ...
    // Setup VT buffering
    uli::runtime::TermCapableCheck::register_resize_handler(
    ...
    if (!final_profile.empty()) {
      std::cout << "[INFO] Loading configuration profile: " << final_profile
                << std::endl;
      if (!uli::runtime::config::ConfigLoader::load_yaml_to_menu_state(
              final_profile, preloaded_state)) {
        std::cerr << "[ERROR] Configuration loading failed. Aborting."
                  << std::endl;
    ...
    if (!final_profile.empty()) {
      if (!uli::runtime::config::ConfigLoader::load_yaml_to_menu_state(
              final_profile, preloaded_state)) {
        std::cerr << "[ERROR] Configuration loading failed for unattended mode."
                  << std::endl;
        return 1;
      }
    }
    ...
    if (preloaded_state.drive.empty() || preloaded_state.drive == "None") {
      uli::runtime::Warn::print_error(
          "Unattended halt: No valid disk target identified in profile.");
      return 1;
    }

    uli::runtime::Warn::print_info("Target verified: " + preloaded_state.drive);
